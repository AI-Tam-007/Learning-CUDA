...
__global__ void softmax_v02_gpu_naive_one_thread_per_row(
    const float* input,
    float* output,
    int rows,
    int cols) {
    ...
    float sum_val = 0.0f;
    for (int col = 0; col < cols; ++col) {
        sum_val += expf(input[row_start + col] - max_val);
    }
...
int main() {
    ...
    for (int i = 0; i < WARMUP_TIMES; ++i) {
        softmax_v02_gpu_naive_one_thread_per_row<<<grid, block>>>(
            d_input,
            d_output,
            ROWS,
            COLS);
    ...
    std::cout << "Kernel: softmax_v02_gpu_naive_one_thread_per_row" << std::endl;
