...
__global__ void matmul_v03_float4_vectorized_load(
    float* A,
    float* B,
    float* C,
    int m,
    ...
    for (int tile = 0; tile < k; tile += BlockK) {
        for (int idx = tid; idx < BlockM * BlockK / 4; idx += thread_num) {
            ...
            if (global_row < m && global_col + 3 < k) {
                FETCH_FLOAT4(As[row][col]) = FETCH_FLOAT4(A[global_row * k + global_col]);
            } else {
                for (int v = 0; v < 4; ++v) {
                    if (global_row < m && global_col + v < k) {
                        As[row][col + v] = A[global_row * k + global_col + v];
                    } else {
                        As[row][col + v] = 0.0f;
                    }
                }
        ...
        for (int idx = tid; idx < BlockK * BlockN / 4; idx += thread_num) {
            ...
            if (global_row < k && global_col + 3 < n) {
                FETCH_FLOAT4(Bs[row][col]) = FETCH_FLOAT4(B[global_row * n + global_col]);
            } else {
                for (int v = 0; v < 4; ++v) {
                    if (global_row < k && global_col + v < n) {
                        Bs[row][col + v] = B[global_row * n + global_col + v];
                    } else {
                        Bs[row][col + v] = 0.0f;
                    }
                }
...
int main() {
    ...
    for (int i = 0; i < WARMUP_TIMES; ++i) {
        matmul_v03_float4_vectorized_load<BM, BN, BK, TM, TN><<<grid, block>>>(d_A, d_B, d_C, M, N, K);
    }
    ...
    std::cout << "Kernel: matmul_v03_float4_vectorized_load" << std::endl;
