...
__global__ void matmul_v01_block_tiling_shared_memory(
    const float* A,
    const float* B,
    float* C,
    int m,
...
int main() {
    ...
    for (int i = 0; i < WARMUP_TIMES; ++i) {
        matmul_v01_block_tiling_shared_memory<BLOCK_SIZE><<<grid, block>>>(d_A, d_B, d_C, M, N, K);
    }
    ...
    std::cout << "Kernel: matmul_v01_block_tiling_shared_memory" << std::endl;
