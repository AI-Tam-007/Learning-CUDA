...
__global__ void vector_add_upgrade(float *gpu_x, float *gpu_y, float *gpu_z, int N_OFFSET)
{
    ...
    {
        float4 gpu_a = reinterpret_cast<float4 *>(gpu_x)[global_idx];    // 向量化的读取A数组数据，隐式地处理了4个数，一次性读4个float，减少访存指令数量。
        float4 gpu_b = reinterpret_cast<float4 *>(gpu_y)[global_idx];    // 向量化的读取B数组数据，隐式地处理了4个数，一次性读4个float，减少访存指令数量。
        float4 gpu_c;                                                    // 定义C数据存储计算后的数
        gpu_c.x = gpu_a.x + gpu_b.x;
        gpu_c.y = gpu_a.y + gpu_b.y;
        gpu_c.z = gpu_a.z + gpu_b.z;
        gpu_c.w = gpu_a.w + gpu_b.w;
        reinterpret_cast<float4 *>(gpu_z)[global_idx] = gpu_c;           // 向量化的写入，一次性写4个float
    }
...
int main()
{
    ...
    // 初学CUDA编程理解：这里除以4是因为后面核函数中用float4*代替了float*，一个float4*中存在4个float*，因此线程数可以减少为原来的1/4，降低了线程需求；如果不除4也不会报错，但是，消耗的资源就不一样了。
    ...
    //printf("grid_one:%d\t%d\n",grid_one, N_OFFSET);
    for (int i = N/N_OFFSET - 1; i >= 0; --i)  // 分块处理超大数据，防止grid过大
    {
	...
	// 用 CUDA Event 测带宽，带宽公式：GB/s = 传输字节数，实际访问量：读x,读y,读z = 12B ≈ 3 x float,所以3 * N_all 
